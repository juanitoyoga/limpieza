...
<body class="min-h-screen bg-gray-50 font-sans antialiased" x-data="{ sidebarOpen: false, userMenuOpen: false, notificationsOpen: false }">
                    ...
                    <div class="relative">
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="hidden md:block text-left">
                                    <div class="font-semibold text-gray-900">
                                        {{ auth()->check() ? auth()->user()->last_name : 'Invitado' }}
                                    </div>
                                    <div class="text-xs text-gray-500">
                                        {{ auth()->check() ? auth()->user()->role_name ?? 'User' : 'No autenticado' }}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div x-show="userMenuOpen"
                            ...
                            <div class="px-4 py-3 border-b border-gray-100">
                                <p class="font-semibold text-gray-900">{{ auth()->check() ? auth()->user()->last_name : 'Invitado' }}</p>
    ...
    {{-- Toast simple para mensajes de Livewire (dispatch('toast', message: '...')) --}}
    <div id="toast-container"
        class="fixed bottom-4 right-4 z-50 flex flex-col gap-2 pointer-events-none"></div>
    ...
    <script>
        ...
        document.addEventListener('livewire:init', () => {
            Livewire.on('toast', (event) => {
                ...
                toast.className = 'pointer-events-auto bg-gray-900 text-white text-sm px-4 py-3 rounded shadow-lg max-w-sm transition-opacity duration-300';
