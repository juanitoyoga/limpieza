...
<div>
                ...
                <tr class="hover:bg-gray-50 transition {{ !$denuncia->activo ? 'bg-gray-50/50 italic text-gray-400' : '' }}">
                    ...
                    <td class="px-4 py-4 text-sm text-center font-medium">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            {{-- Mostrar separador SOLO si hay acciones de flujo --}}
                            @if(in_array($denuncia->estado, ['Pendiente', 'Verificado']))
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                {{-- PENDIENTE: verificar / rechazar --}}
                                @if($denuncia->estado === 'Pendiente')
                                <button wire:click="verificarDenuncia({{ $denuncia->id }})"
                                    class="px-2 py-1 text-xs bg-blue-600 text-white rounded-md hover:bg-blue-700 transition flex items-center gap-1 shadow-xs"
                                    title="Marcar como Verificada">
                                    <x-heroicon-s-check-badge class="w-4 h-4" />
                                </button>

                                <button wire:click="rechazarDenuncia({{ $denuncia->id }})"
                                    class="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded-md hover:bg-red-50 hover:text-red-600 transition flex items-center gap-1"
                                    title="Rechazar denuncia">
                                    <x-heroicon-s-x-mark class="w-4 h-4" />
                                </button>

                                {{-- VERIFICADO: aprobar --}}
                                @elseif($denuncia->estado === 'Verificado')
                                <button wire:click="aprobarDenuncia({{ $denuncia->id }})"
                                    class="px-2 py-1 text-xs bg-green-600 text-white rounded-md hover:bg-green-700 transition flex items-center gap-1 shadow-xs"
                                    title="Aprobar y Emitir Multa Oficial">
                                    <x-heroicon-s-scale class="w-4 h-4" />
                                </button>
                                <button "
                                    class=" px-2 py-1 text-xs bg-white text-white rounded-md hover:bg-green-700 transition flex items-center gap-1 shadow-xs"
                                    title="">
    ...
    @if($confirmingDelete)
    <div class="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-xs z-50 animate-fade-in">
        <div class="bg-white p-6 rounded-xl shadow-xl max-w-md w-full mx-4 border border-gray-100">
            <div class="flex items-center gap-3 mb-3 text-red-600">
                <div class="p-2 bg-red-50 rounded-full">
                    <i class="fas fa-exclamation-triangle text-lg"></i>
                </div>
                <h3 class="text-lg font-bold text-gray-900">¿Eliminar esta denuncia?</h3>
            </div>

            <p class="text-gray-600 mb-6 text-sm">
                Esta acción no se puede revertir. La denuncia con ID <strong>#{{ $deleteId }}</strong> será removida permanentemente del sistema de administración central.
            </p>
            ...
            <div class="flex gap-3 justify-end">
                ...
                <button wire:click="delete"
                    class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition flex items-center text-sm font-medium shadow-sm">
