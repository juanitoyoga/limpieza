@section('page-title', 'Aprobar Resolución')
@section('page-description', 'Aprobación final de la resolución barrial')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-gray-200">
            ...
            <div class="p-6 md:p-8">
                <h2 class="text-lg font-semibold text-gray-800 mb-4">
                    <i class="fas fa-file-signature mr-2 text-green-500"></i>
                ...
                <form wire:submit.prevent="save" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text"
                            value="{{ $resolucion->verificador ? $resolucion->verificador->last_name . ' ' . $resolucion->verificador->first_name : 'N/A' }}"
                            readonly class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 text-gray-600">
                    </div>

                    @if($resolucion->observaciones)
                    <div>
                        <label class="block text-sm font-medium mb-2 text-gray-500">Observaciones previas</label>
                        <textarea readonly rows="3"
                            class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-200 text-gray-600">{{ $resolucion->observaciones }}</textarea>
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        <textarea wire:model="observaciones" rows="3"
                            placeholder="Confirme la aprobación de esta resolución..."
                    ...
                    <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <div class="flex items-start space-x-3">
                            ...
                            <label for="check-resp" class="text-sm text-yellow-900 cursor-pointer">
                                <strong>Declaración Jurada:</strong> Como Presidente barrial, declaro que he revisado esta resolución y asumo la responsabilidad legal de su aprobación.
                            </label>
                        </div>
                    ...
                    <div class="pt-4 flex justify-center space-x-4">
                        <button type="submit" wire:loading.attr="disabled"
                            class="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed
                             text-white rounded-lg font-semibold text-base shadow-md transition-all transform active:scale-95
                            flex items-center justify-center">
                            <span wire:loading.remove class="flex items-center">
                                <i class="fas fa-check-double mr-2"></i> APROBAR
                            </span>
            ...
            <div class="p-6 md:p-8 bg-gray-50 flex flex-col justify-center">
                <div class="max-w-xs mx-auto text-center">
                    <div class="inline-flex items-center justify-center w-20 h-20 bg-green-100 rounded-full mb-6">
                        <i class="fas fa-stamp text-4xl text-green-600"></i>
                    </div>
                    <h3 class="text-xl font-bold text-gray-800 mb-4">Aprobación Final</h3>
                    <ul class="text-left space-y-4 text-sm text-gray-600">
                        <li class="flex items-start">
                            <i class="fas fa-check text-green-500 mt-1 mr-3"></i>
                            <span>Esta es la última etapa del flujo de firma. Tras aprobar, la resolución queda formalmente respaldada.</span>
                        </li>
                        <li class="flex items-start">
                            <i class="fas fa-check text-green-500 mt-1 mr-3"></i>
                            <span>El evento quedará registrado en blockchain de forma inmutable.</span>
                        </li>
                    </ul>
                </div>
            </div>
