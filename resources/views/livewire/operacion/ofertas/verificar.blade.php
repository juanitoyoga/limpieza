...
<div class="max-w-5xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between border-b border-gray-200 pb-4">
        <div class="min-w-0 flex-1">
            <h1 class="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:text-3xl">
                Verificar Oferta: <span class="text-indigo-600">{{ $oferta->codigo }}</span>
            </h1>
        ...
        <div>
            ...
            <dl class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <dd class="text-gray-900 font-semibold">{{ $oferta->proveedor->razon_social ?? 'N/A' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-gray-500 font-medium">Resolución Asociada:</dt>
                    <dd class="text-gray-900">{{ $oferta->resolucion->codigo ?? 'N/A' }}</dd>
                </div>
                <div class="flex justify-between">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tfoot class="bg-gray-50 border-t-2 border-gray-200">
                    <tr>
                        ...
                        <td class="px-6 py-3 text-right font-bold text-indigo-600 text-base">
    ...
    <div class="bg-white shadow rounded-lg p-6 space-y-4">
        <h3 class="text-md font-semibold text-gray-800 border-b pb-2">Confirmación de Verificación</h3>

        <div>
            <label class="flex items-start space-x-3 cursor-pointer">
                <input type="checkbox" wire:model="documentoRevisado" class="mt-1 h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                <span class="text-sm text-gray-700 font-medium">
                    Confirmo que revisé el documento físico/digital y coincide exactamente con el hash registrado en el sistema.
                </span>
            </label>
            @error('documentoRevisado')
        ...
        <div>
            ...
            <textarea id="observaciones" wire:model="observaciones" rows="3" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm p-2 border" placeholder="Ingrese cualquier anotación relevante sobre la verificación..."></textarea>
            @error('observaciones')
            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="pt-4 border-t flex flex-col sm:flex-row justify-end gap-3">
            ...
            <button wire:click="confirmar" class="inline-flex justify-center items-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 text-sm font-medium shadow-sm transition">
